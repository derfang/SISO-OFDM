...
% Features: Parallel Processing + Relative Performance Analysis
clear; clc; close all;
...
N_sym_BER = 5000;      % High symbol count for smooth BER curves
N_sym_Vis = 1000;      % Lower symbol count for Visualization
...
parfor i = 1:length(SNR_Range)
    ...
    for sym_idx = 1:N_sym_BER
        ...
        % MMSE (With Bias Correction Fix: 4 factor)
        N0_eff = (noise_pwr * N_fft) * 4; 
        ...
        local_err_ZF    = local_err_ZF    + biterr(bits_ref, qamdemod(X_hat_ZF, Mod_Order, 'OutputType', 'bit', 'UnitAveragePower', true));
        local_err_MMSE  = local_err_MMSE  + biterr(bits_ref, qamdemod(X_hat_MMSE, Mod_Order, 'OutputType', 'bit', 'UnitAveragePower', true));
        local_err_Adapt = local_err_Adapt + biterr(bits_ref, qamdemod(X_hat_Adapt, Mod_Order, 'OutputType', 'bit', 'UnitAveragePower', true));
    end
...
fprintf('2. Generating Visuals...\n');
h_vis = [1; 0.8; 0.3]; h_vis = h_vis / norm(h_vis);
...
y_chan_vis = y_chan_vis(1:length(x_vec_vis)); 
...
N0_vis     = (noise_pwr_vis * N_fft) * 0.25; % Apply fix here too
...
subplot(1, 3, 1); plot(Y_Rx_Vis(:), '.', 'Color', [0.6 0.6 0.6], 'MarkerSize', 1); title('1. Raw Received'); axis square;
subplot(1, 3, 2); plot(Rx_ZF_Vis(:), '.', 'Color', [0.9 0.2 0.2], 'MarkerSize', 2); hold on; plot(ideal_pts, 'ko', 'MarkerFaceColor','k', 'MarkerSize',5); title('2. Zero Forcing'); axis square;
subplot(1, 3, 3); plot(Rx_MMSE_Vis(:), '.', 'Color', [0.2 0.6 0.2], 'MarkerSize', 2); hold on; plot(ideal_pts, 'ko', 'MarkerFaceColor','k', 'MarkerSize',5); title('3. MMSE'); axis square;

% --- NEW FIGURE 3: Relative Performance Penalty ---
% Formula: (ZF - MMSE) / MMSE
% Avoid division by zero by using max(BER, small_value)
Rel_Metric = (BER_ZF - BER_MMSE) ./ max(BER_MMSE, 1e-10);

figure('Name', 'Relative Performance Analysis', 'Color', 'w', 'Position', [200 200 800 500]);
bar(SNR_Range, Rel_Metric, 'FaceColor', [0.8 0.3 0.3]);
grid on;
xlabel('SNR (dB)'); 
ylabel('Relative Error Increase ( (ZF-MMSE)/MMSE )');
title('Performance Penalty of Zero Forcing');
subtitle('Values > 0 indicate ZF has more errors than MMSE');

% Add text label to the max bar
[max_val, idx] = max(Rel_Metric);
text(SNR_Range(idx), max_val, ['\uparrow ZF has ' num2str(round(max_val)) 'x more errors!'], ...
